...
public class DAOLendingsImpl extends Database implements DAOLendings {
    ...
    public void modificar(Lendings lending) throws Exception {
        try {
            this.Conectar();
            PreparedStatement st = this.conexion.prepareStatement("UPDATE lendings SET user_id = ?, book_id = ?, date_out = ?, date_return = ? WHERE lending_id = ?");
            st.setInt(1, lending.getUser_id());
            st.setInt(2, lending.getBook_id());
            st.setString(3, lending.getDate_out());
    ...
    public Lendings getLending(Users user, Books book) throws Exception {
        ...
        try {
            this.Conectar();
            PreparedStatement st = this.conexion.prepareStatement("SELECT * FROM lendings WHERE user_id = ? AND book_id = ? AND date_return IS NULL ORDER BY lending_id DESC LIMIT 1");
            st.setInt(1, user.getUser_id());
            st.setInt(2, book.getBook_id());
            
            ResultSet rs = st.executeQuery();
            while (rs.next()) {
                lending = new Lendings();
                lending.setId(rs.getInt("lending_id"));
    ...
    public List<Lendings> listar() throws Exception {
        ...
        try {
            this.Conectar();
            PreparedStatement st = this.conexion.prepareStatement("SELECT * FROM lendings ORDER BY lending_id DESC");
